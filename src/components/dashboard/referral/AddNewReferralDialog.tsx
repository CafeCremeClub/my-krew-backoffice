...
const validationSchema = Yup.object({
  referrerId: Yup.string().required('Parrain requis'),
  refereeId: Yup.string().required('Filleul requis'),
  ...
  creationDate: Yup.date().required('Date de création requise'),
...
const AddNewReferralDialog = ({
  ...
  const [consultantSearchQuery, setConsultantSearchQuery] =
    useState<string>('');
  const [referrerSearchQuery, setReferrerSearchQuery] = useState<string>('');

  const { isPending, mutateAsync } = useCreateReferral();
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        toast.error('Échec de la création du parrainage', {
          description:
            'Une erreur est survenue lors de la création du parrainage.',
          position: 'bottom-right',
  ...
  const referrerOptions =
    referrersData?.data?.map((consultant) => ({
      key: consultant.id,
  ...
  const refereeOptions =
    refereesData?.data?.map((consultant) => ({
      key: consultant.id,
      label: `${consultant.firstname} ${consultant.lastname} (${consultant.email})`,
      value: consultant.id,
    })) || [];
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-xl p-1 rounded-[1.25rem] px-2 max-h-[95vh] overflow-y-auto hidden-scrollbar">
        <div className="bg-white rounded-2xl overflow-hidden flex flex-col gap-10 py-6 px-2">
          <DialogHeader>
            <DialogTitle>Ajouter une nouvelle cooptation</DialogTitle>
            <DialogDescription>
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-6">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              {formik.touched.referrerId && formik.errors.referrerId && (
                <CustomErrorIndicator message={formik.errors.referrerId} />
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              {formik.touched.refereeId && formik.errors.refereeId && (
                <CustomErrorIndicator message={formik.errors.refereeId} />
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              {formik.touched.creationDate && formik.errors.creationDate && (
                <CustomErrorIndicator message={formik.errors.creationDate} />
              )}
            </div>
