...
export function handleSignupError(error: unknown, setError?: (msg: string) => void): string {
    if (error instanceof AxiosError) {
        if (error.status === 500) {
            if (error.response && error.response.data.name === "OtpInvalid") {
                const message = "Le code de vérification est invalide ou a expiré. Veuillez demander un nouveau code.";
                setError?.(message);
                return message;
            }
